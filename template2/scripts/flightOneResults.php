...
<div class="a">
		...
		<form name="oneFlight" method="post" action="flightOneResults.php" enctype="multipart/form-data">
		
		 <label for="Airport">From</label>
		                 <input class="ui-widget" value="<?php echo $Airport ?>" id="Airport" name="Airport">
							
							
		&nbsp &nbsp <label for="Destination">To</label>
		                 <input class="ui-widget" value="<?php echo $Destination ?>" id="Destination" name="Destination">
		...
		&nbsp &nbsp<input type="number" name="quantity2" value="<?php echo $quantity2?>" id="child" min="0" max="4">
		 ...
		 &nbsp &nbsp <input class="new" type="submit" value="SEARCH FLIGHTS" name="SUBMIT"></input>
...
{
	if($row['Airport']==$Airport && $row['Destination']==$Destination)
	{
	  $rtid=$row['RtID'];
	  //echo $rtid;
	  ...
	  {
		  $date = date_create($row['FlightDate']);
		  //echo date_format($date,'m/d/Y');
	  if($dat==date_format($date,'m/d/Y')&&$row['AirCraft']==$rtid)
	  { 
          //echo "hi";
	  ...
	  while($row=mysqli_fetch_assoc($result2)){
		  if($acid==$row['AcID']){?>
			  <center>	
						...
						<tr>
						       <td><?php echo $row['Company'];?></td><td><?php echo $Airport;?></td>
							   <td><?php echo $Destination;?></td><td><?php echo $departure;?></td><td><?php echo $arrival;?></td>
							   <td><?php echo $row['availability'];?></td><td><?php echo $fare;?></td><td><?php echo "<a href='flightOneBook.php?id=".$ffid."'>book&nbsp &nbsp &nbsp";?></td>
						</tr>
                         </table>	
